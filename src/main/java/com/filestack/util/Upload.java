...
public class Upload {
  ...
  public Flowable<Progress<FileLink>> runAsync() {
    Flowable<Progress<FileLink>> start = Flowable
    ...
    Flowable<Progress<FileLink>> transfer = Flowable.empty();
    for (int i = 0; i < CONCURRENCY; i++) {
      UploadTransferFunc func = new UploadTransferFunc(this, i);
      Flowable<Progress<FileLink>> sub = Flowable
          .create(func, BackpressureStrategy.BUFFER)
          .subscribeOn(Schedulers.io());
      transfer = transfer.mergeWith(sub);
    }

    Flowable<Progress<FileLink>> complete = Flowable
